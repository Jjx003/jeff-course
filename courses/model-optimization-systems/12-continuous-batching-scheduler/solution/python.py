...
A continuous-batching decode scheduler driving a real paged KV cache, plus the
equivalence test that proves batching did not change any request's output.

Graded output goes to stdout and is CPU/float32/seed-0 deterministic. Wall-clock
measurement goes to stderr.
"""
...
def allocate_block(cache: dict, rid: str) -> int:
    """Pop the lowest free physical block and append it to rid's block table."""
    block = cache["free"].pop(0)
    cache["table"][rid].append(block)
    cache["allocations"].append((block, rid))
    return block
...
def free_request(cache: dict, rid: str) -> None:
    ...
    The blocks are not zeroed. Stale keys are harmless as long as every read is
...
def write_kv(cache: dict, rid: str, pos: int, k: torch.Tensor, v: torch.Tensor) -> None:
    ...
    slot = pos % block_size
    cache["k"][block, slot] = k
    cache["v"][block, slot] = v
...
def decode_step(
    ...
    for i, rid in enumerate(active_ids):
        length = lengths[i]
        k, v = gather_kv(cache, rid, length)
        keys[i, :length] = k
        values[i, :length] = v
        mask[i, :length] = True
    ...
    return out.reshape(batch, D_MODEL), block_reads
...
def replay_single(request: dict, qkv: dict, block_size: int) -> torch.Tensor:
    ...
    rid = request["id"]
    q, k, v = qkv[rid]
    cache = make_cache(blocks_needed(request, block_size), block_size)
    state = admit(cache, request, qkv, block_size)

    rows = []
    for _ in range(request["gen"]):
        pos = state["length"]
        ensure_capacity(cache, rid, pos)
        write_kv(cache, rid, pos, k[pos], v[pos])
        state["length"] = pos + 1
        out, _ = decode_step(cache, [rid], [state["length"]], q[pos].unsqueeze(0))
        rows.append(out[0])
...
def fragmentation(blocks_in_use: int, block_size: int, live_tokens: int) -> tuple[int, int, float]:
    ...
    allocated = blocks_in_use * block_size
    wasted = allocated - live_tokens
    return allocated, wasted, 100.0 * wasted / allocated
