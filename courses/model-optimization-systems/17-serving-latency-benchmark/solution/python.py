"""
Reference solution for module 17.
...
def nearest_rank_percentile(sorted_values: list[float], q: float) -> float:
    ...
    """
    n = len(sorted_values)
    rank = math.ceil(Fraction(str(q)) / 100 * n)
...
def simulate_fcfs(interarrivals: list[float], services: list[float]) -> list[float]:
    ...
    server_free = 0.0
    waits = []
    for gap, service in zip(interarrivals, services):
        now += gap
...
def simulate_closed_loop(
    ...
    for client in range(n_clients):
        heapq.heappush(pending, (rng.expovariate(1.0 / think_mean), client))
    server_free = 0.0
    waits = []
    finish = 0.0
    for _ in range(n_requests):
        ...
        service = sample_service(rng)
        finish = start + service
        server_free = finish
        waits.append(start - arrival)
        heapq.heappush(pending, (finish + rng.expovariate(1.0 / think_mean), client))
    return waits, finish
