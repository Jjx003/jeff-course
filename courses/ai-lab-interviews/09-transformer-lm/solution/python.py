...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, cos, sin):
        # Pre-norm: the residual adds the UNNORMALIZED x, keeping the stream
        # a clean additive path from input to output.
...
class TransformerLM(nn.Module):
    ...
    def forward(self, ids):
        ...
        x = self.norm_f(x)
        # Weight tying: the same tensor serves as embedding and output head,
        # so it receives the sum of both paths' gradients.
