...
There are SIX bugs in the MODEL section. Every one of them is something that
has shipped in real code, and none of them raises an exception — a broken
transformer trains, it just trains to the wrong thing.
...
def causal_mask(seq, device=None):
    ...
    return torch.ones(seq, seq, dtype=torch.bool, device=device).triu()
...
def split_heads(x, n_heads):
    ...
    return x.view(b, n_heads, s, head_dim)
...
def lm_loss(logits, ids):
    ...
    return F.cross_entropy(logits.reshape(-1, vocab), ids.reshape(-1))
