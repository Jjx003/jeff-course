...
def forward(params, x, y):
    ...
    cache = {"x": x, "y": y, "z1": z1, "h1": h1, "probs": np.exp(logp)}
    return loss, cache


def backward(params, cache):
    """Hand-written gradients for every parameter."""
    x, y, z1, h1, probs = cache["x"], cache["y"], cache["z1"], cache["h1"], cache["probs"]
    n = x.shape[0]

    # dL/dz2 = (softmax - onehot) / N. The 1/N is the mean reduction, folded
    # in here exactly once so no later step has to remember it.
    dz2 = probs.copy()
...
def numerical_gradient(params, x, y, name, step=FD_STEP):
    ...
    for i in range(flat.size):
        ...
        flat[i] = original
        flat_grad[i] = (loss_plus - loss_minus) / (2.0 * step)
