...
def merge_heads(x: torch.Tensor) -> torch.Tensor:
    ...
    # transpose leaves the tensor non-contiguous; view requires contiguous.
    return x.transpose(1, 2).contiguous().view(b, s, h * head_dim)


def causal_mask(seq: int, device=None) -> torch.Tensor:
    """(S, S) boolean, True where attention is allowed. Diagonal included."""
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = attention(q, k, v, causal_mask(seq, x.device))
