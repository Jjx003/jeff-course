...
Therefore, every positive solution $(x, y)$ to Pell's equation appears among the convergents of the continued fraction expansion of $\sqrt{D}$.
...
where $Q_{k+1}$ is a positive integer that appears in the standard complete-quotient calculation for $\sqrt{D}$. At the period boundaries, $Q_{k+1} = 1$, so the corresponding convergents solve either $x^2 - Dy^2 = 1$ or $x^2 - Dy^2 = -1$ depending on the parity of the period.
