...
### 1. Commutativity
$$ (f * g)(n) = \sum_{d|n} f(d)g(n/d) = \sum_{ab=n} f(a)g(b) = \sum_{ba=n} g(b)f(a) = (g * f)(n) $$
...
$$ f^{-1}(1) = \frac{1}{f(1)} $$
$$ f^{-1}(n) = \frac{-1}{f(1)} \sum_{d|n, d > 1} f(d)f^{-1}(n/d) $$
...
**Theorem:** If $f$ is a multiplicative function and $\sum |f(n)|n^{-\sigma}$ converges, then:
$$ \sum_{n=1}^{\infty} \frac{f(n)}{n^s} = \prod_{p} \sum_{k=0}^{\infty} \frac{f(p^k)}{p^{ks}} $$

**Proof Strategy:**
Consider the finite product over primes $p \le X$:
$$ P(X) = \prod_{p \le X} \left( 1 + \frac{f(p)}{p^s} + \frac{f(p^2)}{p^{2s}} + \dots \right) $$
...
where $S_X$ is the set of integers whose prime factors are $\le X$. As $X \to \infty$, $S_X \to \mathbb{N}$. Since the series converges absolutely, the limit exists and equals the Dirichlet series. $\square$
...
1. **Möbius Inversion:** Since $\mu * \mathbf{1} = \epsilon$, where $\mathbf{1}(n)=1$:
