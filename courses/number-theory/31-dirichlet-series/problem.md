...
The natural "product" for these functions is not point-wise multiplication, but **Dirichlet convolution**, denoted by $(f * g)$:
...
The magic of Dirichlet series lies in how they interact with convolution. If $F(s)$ is the series for $f$ and $G(s)$ is the series for $g$, then their product $F(s)G(s)$ is the Dirichlet series for $(f * g)$:

$$ F(s)G(s) = \left( \sum_{n=1}^{\infty} \frac{f(n)}{n^s} \right) \left( \sum_{m=1}^{\infty} \frac{g(m)}{m^s} \right) = \sum_{k=1}^{\infty} \frac{(f * g)(k)}{k^s} $$

## Euler Products

For **multiplicative** arithmetic functions ($f(mn) = f(m)f(n)$ for $\gcd(m,n)=1$), the Dirichlet series can be written as an infinite product over primes:
