...
Then $x + y = p^{v_x} \left( \frac{a}{b} + p^{v_y - v_x} \frac{c}{d} \right) = p^{v_x} \frac{ad + p^{v_y - v_x} bc}{bd}$.
...
Taking the exponent: $|x+y|_p = p^{-v_p(x+y)} \le p^{-\min(v_p(x), v_p(y))} = \max(|x|_p, |y|_p)$. $\square$
...
where $a_i \in \{0, 1, \dots, p-1\}$. This looks like a base-$p$ expansion that goes "to the left" infinitely.
